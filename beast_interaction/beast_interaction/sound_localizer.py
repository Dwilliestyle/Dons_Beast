...
Uses the Beast's stereo soundboard (plughw:CARD=Device,DEV=0) to estimate
the horizontal angle to a sound source at the moment of wake word detection.
"""
...
SAMPLE_RATE       = 48000    # Hz — native rate of the USB PnP soundboard
...
def capture_stereo_snapshot(duration=1.0, device='plughw:CARD=Device,DEV=0'):
    ...
    cmd = [
        ...
        '-c', '2',
        '-r', str(SAMPLE_RATE),
        '-d', str(int(duration)),   # must be an integer — arecord rejects '1.0'
        filename
    ]
    try:
        ...
        if not os.path.exists(filename):
            print(f'[sound_localizer] capture error: recording failed, no file created')
            return None, None
